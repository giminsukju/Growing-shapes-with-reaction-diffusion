...
Eigen::VectorXd original_scaling_0;
Eigen::VectorXd original_scaling_1;
...
void implicitReactionDiffusionGrayScott(float numSteps, float timeStep, double F_val, double k_val) {
    ...
    double mesh_size = igl::bounding_box_diagonal(meshV); //largest EUclidean distance in the given mesh
    double desired_dist = mesh_size / 20;
    std::cout << mesh_size << "  " << desired_dist << std::endl;
    ...
    original_scaling_0 = U; //update scaling as a global variable
    original_scaling_1 = V;
    auto temp = polyscope::getSurfaceMesh("input mesh");
    auto mesh_0 = temp->addVertexScalarQuantity("morphogen 0", U);
    auto mesh_1 = temp->addVertexScalarQuantity("morphogen 1", V);
...
void growShape(int n_iteration, double alpha, double beta, double min_scaling, double max_scaling, int morphogen) { //TODO: take mesh file as an input
    std::cout << "shape growing parameters: " << n_iteration << " " << alpha << " " << beta << " " << min_scaling << " " << max_scaling << std::endl;

    Eigen::MatrixXd V_init = meshV;
    Eigen::MatrixXi F_init = meshF;

    // manual scaling
    // grid: set non-uniform scaling, inner area scaling > outer area scaling
    ...
    // spot: set non-uniform scaling, spot face scaling > spot body scaling
    //Eigen::VectorXd scaling = Eigen::VectorXd::Constant(V_init.rows(), 1, 1.2);
    ...
    // scaling based on reaction-diffusion 
    Eigen::VectorXd scaling(V_init.rows(), 1);
    if (morphogen == 0) {
        scaling = ((max_scaling - min_scaling) * original_scaling_0) + Eigen::VectorXd::Constant(V_init.rows(), 1, min_scaling);
    }
    else {
        scaling = ((max_scaling - min_scaling) * original_scaling_1) + Eigen::VectorXd::Constant(V_init.rows(), 1, min_scaling);
    }
...
void callback() {
    ...
    static int morphogen;
    ImGui::SliderInt("switch morphogens", &morphogen, 0, 1);
    ...
    if (ImGui::Button("growing shapes refresh")) {
        growShape(growing_n_iteration, alpha, beta, min_scaling, max_scaling, morphogen);
    }
}
